...
export default function UserActions({id, claim}: {id: string; claim: any}) {
    return (
        <Tabs defaultValue="Package Details" className="p-4">
            <TabsList>
                {claim.rga ? <TabsTrigger value="packageDetails">Package Details</TabsTrigger> : null}
            ...
            <TabsContent value="packageDetails" className="px-3">
                {claim.shipment ? <p>Package details and arrival method has been sent to MSP.</p> : <ShippingForm id={id} />}
            </TabsContent>
